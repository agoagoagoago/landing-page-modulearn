        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-8 max-w-5xl mx-auto">
          {plans.map((plan) => {
            ...
            return (
              ...
              >
                {plan.popular && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-emerald-500 text-white text-[10px] sm:text-xs font-bold px-3 sm:px-4 py-1 rounded-full uppercase tracking-wider whitespace-nowrap">
                    Most Popular
                  </div>
                )}
                ...
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center mb-4 sm:mb-5 ${
                  ...
                  <Icon className={`h-5 w-5 sm:h-6 sm:w-6 ${plan.popular ? 'text-emerald-400' : 'text-emerald-600'}`} />
                </div>

                <h3 className={`text-lg sm:text-xl font-bold mb-1.5 sm:mb-2 ${plan.popular ? 'text-white' : 'text-navy-900'}`}>
                  {plan.name}
                </h3>
                <p className={`text-xs sm:text-sm mb-4 sm:mb-6 ${plan.popular ? 'text-navy-300' : 'text-navy-500'}`}>
                  {plan.description}
                </p>

                <div className="mb-4 sm:mb-6">
                  <span className={`text-2xl sm:text-3xl font-bold ${plan.popular ? 'text-white' : 'text-navy-900'}`}>
                    {plan.price}
                  </span>
                  <span className={`text-xs sm:text-sm ${plan.popular ? 'text-navy-300' : 'text-navy-400'}`}>
                    {' '}{plan.period}
                  </span>
                </div>
                ...
                <ul className="space-y-2.5 sm:space-y-3 mb-6 sm:mb-8 flex-1">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5 sm:gap-3">
                      <Check className={`h-4 w-4 sm:h-5 sm:w-5 mt-0.5 shrink-0 ${plan.popular ? 'text-emerald-400' : 'text-emerald-500'}`} />
                      <span className={`text-xs sm:text-sm ${plan.popular ? 'text-navy-200' : 'text-navy-600'}`}>{f}</span>
                    </li>
                  ))}
                </ul>
                ...
                <a
                  href="#contact"
                  className={`w-full inline-flex items-center justify-center gap-2 py-3 px-4 sm:px-6 rounded-xl text-sm font-semibold transition-colors ${
                    plan.popular
