        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-8">
          {features.map((feature, i) => {
            ...
            return (
              <div
                key={feature.title}
                className="group relative bg-navy-50 hover:bg-navy-900 rounded-2xl p-6 sm:p-8 transition-colors duration-300 border border-navy-100 hover:border-navy-700"
              >
                <div className="absolute top-0 left-6 right-6 sm:left-8 sm:right-8 h-px bg-gradient-to-r from-transparent via-emerald-500 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />

                <div className="w-12 h-12 sm:w-14 sm:h-14 bg-emerald-100 group-hover:bg-emerald-500/20 rounded-xl flex items-center justify-center mb-4 sm:mb-6 transition-colors">
                  <Icon className="h-6 w-6 sm:h-7 sm:w-7 text-emerald-600 group-hover:text-emerald-400 transition-colors" />
                </div>

                <span className="text-xs font-bold text-emerald-500 tracking-widest uppercase mb-1.5 sm:mb-2 block">
                  Step {i + 1}
                </span>

                <h3 className="text-lg sm:text-xl font-bold text-navy-900 group-hover:text-white mb-2 sm:mb-3 transition-colors">
                  {feature.title}
                </h3>

                <p className="text-sm sm:text-base text-navy-500 group-hover:text-navy-300 mb-3 sm:mb-4 transition-colors leading-relaxed">
                  {feature.description}
                </p>
